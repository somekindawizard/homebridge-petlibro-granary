...
A UX and polish pass focused on how the plugin presents itself in the Home
app and the Homebridge Config UI. No breaking changes for existing users;
all new features default to their backward-compatible behavior.

### Added

- **Configurable HomeKit services.** New `enabledServices` config array lets you choose exactly which tiles appear in the Home app. Battery is always present. All 9 optional services default to enabled (backward compatible). Disabled services are actively removed from cached accessories so they disappear on restart.
- **Pet-aware service labels.** When a pet is bound to the feeder in the PETLIBRO app, service names use the pet's name: "Feed Mochi" instead of "Kitchen Feeder Feed Now", "Mochi Food Low" instead of "Kitchen Feeder Food Low". Falls back to the device name when no pet is bound or the API data is unavailable.
- **Cached service name updates.** Service display names are now pushed to both `displayName` and `Characteristic.Name` on every startup, so users upgrading from older versions (or renaming a pet) see the new labels without needing to remove and re-add the accessory.
- **Timer cleanup on accessory removal.** New `destroy()` method on the accessory handler clears all outstanding timers (feed reset, desiccant reset, recent feed pulse). Called during orphan pruning and Homebridge shutdown so timer callbacks never fire against stale service references.
- **Accessory-layer test suite.** New `accessory.test.ts` with 25+ test cases covering `computeLowBattery` (NOT_CHARGEABLE suppression, percentage threshold, batteryState strings, boundary at 20%), `computeChargingState`, `computeDesiccantChangeIndication`, `computeDesiccantLifeLevel` (clamping, null, overflow, invalid config), service opt-in (all enabled, subset, stale removal), pet-aware naming (pet bound, no pet, petName fallback, whitespace trimming), `destroy()` safety, and `refreshCharacteristics` with disabled services.
- **`BoundPet` type** in `petlibroApi.ts` for the bound pets API response shape.
- **`primaryPetName` getter** on the Device base class, reading the first bound pet's name with fallback to `petName` field.

### Changed

- **Improved config schema UX.** Password field is now masked (`x-schema-form: { type: "password" }`). Fields are grouped into Account, Feeder Settings, HomeKit Services, and Advanced (collapsed) sections via a `layout` array. Service selection uses checkboxes. Region dropdown now explains that only US is currently supported.
- **Friendlier service names.** "Dispenser" renamed to "Feeder Jam", "Recent Feed" to "Last Fed", "Reset Desiccant" to "Replace Desiccant", "Feeding Schedule" to "Schedule". These labels better communicate what each tile does to non-technical users.
- **README improvements.** Added npm version / build status / license badges. Added Installation section (Homebridge UI + CLI). Added "Choosing which tiles to show" section with a service key reference table. Added "What's new in 0.5.0" summary. Softened security notes tone.
- **Shutdown handler** now calls `destroy()` on all accessory handlers before the logout attempt.
- **Orphan pruning** now calls `destroy()` on handlers for removed devices before unregistering their accessories.
- **`// sic` comment** on the `getfeedingplantoday` key in `DeviceRawData` to document that the unconventional casing is ported verbatim from the Python integration.
...
- **Login backoff with circuit breaker.** Failed logins now back off exponentially (5s to 5min, +/-20% jitter). After 6 consecutive failures the breaker trips and the plugin stops trying until restart -- no more re-login storm hammering PETLIBRO on a typo.
